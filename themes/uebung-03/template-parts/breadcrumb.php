...
function get_breadcrumb() {
	...
	$breadcumb_markup = '<nav class="breadcrumb"><div class="wrap"><span class="show-for-sr">Sie befinden sich hier: </span>
    <ol id="breadcrumb-menu" itemprop="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">';
	...
	if ( ! is_home() && ! is_front_page() || is_paged() ) {
		...
		$breadcumb_markup .= $li1 . '<meta itemprop="position" content="1"><a itemprop="item" href="' . get_option( 'home' ) . '"><span itemprop="name">Startseite</span></a>' . $li2;
		if ( is_page() && ! $post->post_parent ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				$breadcumb_markup .= $li1 . '<meta itemprop="position" content="'.$i++.'"><a itemprop="item" href="'.$home_link.'/'.$slug['slug'].'"><span itemprop="name">'.$post_type->labels->singular_name.'</span></a>' . $li2;
				$breadcumb_markup .= $li1 . '<meta itemprop="position" content="'.$i.'"><span itemprop="name">'.get_the_title().'</span>' . $li2;
			} else {
				$i = 2;
				...
				if ( $this_cat->parent != 0 ) {
					...
					while ( $this_cat->parent ) {
						$parent_cat = get_category( $this_cat->parent );
						$categories[] = '<a itemprop="item" href="'.get_category_link( $parent_cat->term_id ).'"><span itemprop="name">'.$parent_cat->name.'</span></a>';
						$this_cat = $parent_cat;
					}
				...
				$breadcumb_markup .= $li1 . '<meta itemprop="position" content="'.$i++.'"><a itemprop="item" href="'.get_category_link( $cat->term_id ).'"><span itemprop="name">'.$cat->name.'</span></a>' . $li2;
	...
	$breadcumb_markup .= '</ol></div></nav>';

	return array( 'markup' => $breadcumb_markup, 'ids' => $breadcrumb_ids );
}
